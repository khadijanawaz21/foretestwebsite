 ...
 * writes on every successful build (generator/lib/redirects.mjs),
 * statically imported here so its contents are inlined into the edge
 * bundle at deploy time. No Supabase query and no filesystem read at
 * request time — the Edge Runtime has no `fs`, and none is needed.
 * Decision logic itself lives in redirect-lookup.mjs (kept out of
 * generator/, which this phase must not modify) so it's unit-testable
 * under plain Node.
 */
import redirectManifest from './generator/.cache/redirect-manifest.json' with { type: 'json' };
